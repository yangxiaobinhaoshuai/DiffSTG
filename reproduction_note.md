...
| PEMS04 | 1.01× | 2.06× | 1.65 GB | 7.9~16.8 h *(实测)* | 2.7 h *(实测)* | **46.8 h** *(实测)* |
| PEMS03 | 0.98× | 2.45× | 1.92 GB | ~18 h | ~5.0 h | **~61 h** |
| PEMS07 | 1.79× | 7.41× | 4.73 GB | ~48 h | ~16 h | 超单 seed 墙钟上限，不跑 |

PEMS04 已跑完（2026-09-01），实测 46.8 h 对估算 44 h。采样成本估得准（test 2.7 h/seed 与
估算一致）；训练时长的不确定性全在早停 —— 三个 seed 的 best_epoch 是 215/179/96，训练段
因此从 7.9 h 到 16.8 h 不等，跑满 300 epoch 约 15.3 h（单 epoch ~183 s = 训练 73 s +
512 窗口验证 110 s）。PEMS03 按同样口径估 ~61 h，仍在墙钟上限内。
...
## 2026-09-01 PEMS04 三 seed（原文没有，无对照）

`run_3seeds_PEMS04_start0_20260830-155713`，code commit `913eb34`，`START_EPOCH=0`
`VAL_SUBSET=512` `NO_SHUTDOWN=1`，2026-08-30 15:57 → 2026-09-01 14:47，总计 46:50:36。
三个 seed 全部正常早停，无失败、无中断。**CSV 里的行由 `train.py` 直接以 `ddpm`-200 写出，
不再需要从 checkpoint 补测**（PEMS08 那轮是事后补的）。

全量 test 集 3376 个窗口（`16992 - 13593 - 24 + 1`，与 PEMS08 的 3549 同一口径）、
`n_samples=8` 样本均值。

| seed | 总耗时 | 其中 test | best_epoch | MAE | RMSE | MAPE | CRPS | MIS |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2022 | 19:31:38 | 2.71 h | 215 | 22.894 | 35.271 | 16.159 | 0.0821 | 253.49 |
| 2023 | 16:43:31 | 2.71 h | 179 | 22.782 | 35.138 | 15.819 | 0.0817 | 254.36 |
| 2024 | 10:34:10 | 2.66 h | 96 | 22.545 | 34.958 | 15.695 | 0.0810 | 249.93 |
| **mean ± sd** | — | — | — | **22.74 ± 0.18** | **35.12 ± 0.16** | 15.89 ± 0.24 | **0.0816 ± 0.0006** | 252.59 ± 2.35 |

几点：

- **没有可对照的已发表数字**，PEMS08 的 17.92（对原文 17.68）是唯一的协议锚点。这一节的
  数字只能当作"同一份忠实 pipeline 在另一个路网上的 baseline"，不能称为复现。
- **种子 sd 0.18 MAE（0.8%）**，比 PEMS08 的 0.11（0.6%）略大，同量级。n=3 的 sd 估得很糙，
  别当精确数字用。
- 总耗时的差异全来自早停：best_epoch 215/179/96，采样成本每个 seed 是一样的。
- 相对误差 MAPE 15.9% 高于 PEMS08 的 11.5%；PEMS04 的绝对 MAE 更大部分来自流量尺度更大，
  两个数字不能直接横比。
- CSV 里那行 MAE 3063 的是 2-epoch 冒烟（`test_` 前缀、`is_test=True`），不是结果。

**事后发现的一个坑**：这一轮跑完，三个 best checkpoint 全是 untracked —— `d85584f` 让
...
## 2026-09-01 RNG 对照实验：冻结噪声值 0.33 MAE

量化上一节那个原版行为。**单变量对照**：除 `--rng_restore 1` 外全部与已完成的 PEMS08
seed 2022 一致（`START_EPOCH=0` `VAL_SUBSET=512`、同一 `default_config`、同一 `ddpm`-200
测试协议、同一份全量 test 集 3549 窗口、`n_samples=8`）。

`PEMS08_start0_rngfix_20260901-154802`，code commit `c4416f0`，
2026-09-01 15:48 → 21:24，耗时 05:35:20，`status=success`。

| seed 2022 | best_epoch | MAE | RMSE | MAPE | CRPS | MIS | 耗时 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 原版（RNG 冻结） | 173 | 17.899 | 27.309 | 11.317 | 0.06110 | 215.39 | 08:59 |
| **`--rng_restore 1`** | **78** | **17.566** | **26.810** | **11.219** | **0.05990** | **208.33** | **05:35** |
| 差 | −95 | **−0.333 (−1.9%)** | −0.499 (−1.8%) | −0.098 (−0.9%) | −0.0012 (−2.0%) | −7.06 (−3.3%) | −3:24 |

**五个指标全部变好，方向与预期一致**：冻结噪声确实在削弱 DiffSTG，不是无害的实现细节。

两个附带发现：

- **收敛快一倍多**（best_epoch 173 → 78，总耗时 8:59 → 5:35）。每 epoch 仍是 169 s，省下来的
  全是 epoch 数。噪声多样性回来之后，梯度不再是同一批固定三元组的重复，早停触发得早得多。
  这意味着**切到修正版反而更便宜**，不是更贵。
- **17.566 已低于原文的 17.68**。所以现在有两个数、含义不同：`ddpm`-200 + 原版 RNG 的
  **17.92 ± 0.11** 是「按发布代码复现原文」的锚点（对 17.68 高 0.24）；修正 RNG 后的 17.57
  是「这份代码实际能到的水平」。两个都要留着，不能只报一个。

**统计强度：这一轮是 n=1，只能定量级，不能定论。** 原版协议下 PEMS08 的种子 sd 是 0.11 MAE，
两个单次运行之差的 sd 约 0.16，所以 0.333 大约是 2.1σ —— 够说明方向，不够当作精确幅度。
要坐实得补 seed 2023/2024（各约 5.6 h）。

**对论文的影响（待定，见 TODO）**：如果 baseline 用修正版，PEMS08/PEMS04 六次运行都得重跑；
如果继续用原版，必须在 limitation 里写明这 0.33 并说明**自己的方法也跑在同一规则下**。
两边一起改或两边都不改，绝不能只改一边。
